...
class prediction(object):
    ...
    def preprocess(self):
       # self.df.drop(self.df.loc[self.df['Carriageway_Hazards']=='Data missing or out of range'].index, inplace=True)
        ...
        encoding_light = {"Light_Conditions": {"Daylight": 0, "Darkness - lights lit": 1, "Darkness - no lighting": 1,
                                               "Darkness - lighting unknown": 1, "Darkness - lights unlit": 1, "Data missing or out of range": 0}}
        self.df.replace(encoding_light, inplace=True)

        encoding_day_of_week = {"Day_of_Week": {"Saturday": 1, "Sunday": 1,
        ...
        encoding_Special_Conditions_at_Site = {"Special_Conditions_at_Site": {"None": 0, "Roadworks": 1, "Oil or diesel": 1, "Mud": 1, "Road surface defective": 1,
                                                                              "Auto traffic signal - out": 1, "Road sign or marking defective or obscured": 1, "Auto signal part defective": 1, "Data missing or out of range": 0}}
        self.df.replace(encoding_Special_Conditions_at_Site, inplace=True)

        encoding_1st_road_class = {"1st_Road_Class": {
            "A": 1, "A(M)": 1, "B": 2, "C": 3, "Motorway": 4, "Unclassified": 1}}
       ...
       # self.df.drop(self.df.loc[self.df['Urban_or_Rural_Area']=='Unallocated'].index, inplace=True)
        ...
        self.df['Speed_limit'].fillna(
            (self.df['Speed_limit'].mean()), inplace=True)

        self.df['Time'].fillna(0, inplace=True)
        self.df['Time'] = self.df['Time'].apply(self.period)

        accident_severity = {"Accident_Severity": {
            "Serious": 1, "Fatal": 1, "Slight": 0}}
    ...
    def splitEqualRatio(self):
        ...
        X_Serious_train, X_Serious_test, Y_Serious_train, Y_Serious_test = train_test_split(
            X_Serious, Y_Serious, test_size=0.25)

        X_Slight = XY_Slight[self.cols]
        Y_Slight = XY_Slight[['Accident_Severity']]
        X_Slight_train, X_Slight_test, Y_Slight_train, Y_Slight_test = train_test_split(
            X_Slight, Y_Slight, test_size=0.25)

        X_Fatal = XY_Fatal[self.cols]
        Y_Fatal = XY_Fatal[['Accident_Severity']]
        X_Fatal_train, X_Fatal_test, Y_Fatal_train, Y_Fatal_test = train_test_split(
            X_Fatal, Y_Fatal, test_size=0.25)
    ...
    def analyzeData(self):
        ...
        Count_Slight_accidents = len(
            self.df[self.df["Accident_Severity"] == "Slight"])  # Slight accidents
        Count_Serious_accidents = len(
            self.df[self.df["Accident_Severity"] == "Serious"])  # Serious accidents
        Count_Fatal_accidents = len(
            self.df[self.df["Accident_Severity"] == "Fatal"])  # Fatal accidents

        Percentage_of_Slight_accidents = Count_Slight_accidents / \
            (Count_Slight_accidents+Count_Serious_accidents+Count_Fatal_accidents)
        print("percentage of slight accident is",
              Percentage_of_Slight_accidents*100)
        Percentage_of_Serious_accidents = Count_Serious_accidents / \
            (Count_Slight_accidents+Count_Serious_accidents+Count_Fatal_accidents)
        print("percentage of serious accident is",
              Percentage_of_Serious_accidents*100)
        Percentage_of_Fatal_accidents = Count_Fatal_accidents / \
            (Count_Slight_accidents+Count_Serious_accidents+Count_Fatal_accidents)
        print("percentage of fatal accident is",
              Percentage_of_Fatal_accidents*100)

    def printAnalysis(self, actual, pred):
        print("accuracy_score for gaussianClassifier: %f" %
    ...
    def predictResult(self, data):
        ...
        test = pd.DataFrame.from_dict(
            inputData, orient='index', columns=self.cols)
    ...
    def __init__(self):
        ...
        self.cols = ['1st_Road_Class', 'Carriageway_Hazards', 'Day_of_Week', 'Junction_Control', 'Junction_Detail', 'Light_Conditions',
                     'Road_Surface_Conditions', 'Road_Type', 'Special_Conditions_at_Site', 'Speed_limit', 'Time', 'Urban_or_Rural_Area', 'Weather_Conditions']
